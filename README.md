...
2. Extract metadata: large dataset, so we retrieve some fields (e.g., author, title, isbn). In total, 3,723,104 records.
3. Query datos.bne.es: when you run many queries, the repository stops working. As a result, we try with a second approach as is described in step 4. 
...
6. Match records by isbn and get Resource id. In total, 2,713,506 matched records using isbn. See a sample of records in this file [first_500_rows.csv](first_500_rows.csv)
...
In order to load the dataset in a local environment, we employed the scripts: [process-bne-fuseki.py](process-bne-fuseki.py) and [sparqlBne.py](sparqlBne.py)
